...
index.normalize <- function (index) {
    ##Standardize to normal distribution (quantiles of a normal distribution

    ##check that index is numeric
    if (!is.numeric (index)) stop ("index is not numeric")  
...
##' @seealso \code{\link{transferIndex}}, \code{\link{uvGsa}},
##' \code{\link{mdPat}}
##'
